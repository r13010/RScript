...
select_option() {
    ...
    key_input()         {
                        ...
                        if [[ $key = "w" ]]; then echo up; fi;
                        if [[ $key = "s" ]]; then echo down; fi;
                        if [[ $key = "a" ]]; then echo left; fi;
                        if [[ $key = "d" ]]; then echo right; fi;
                        if [[ $key = "z" ]]; then echo all; fi;
...
2) 
while true; do
  echo -ne "] rscript: Please enter your luks password: \n"
  read -s luks_password # read password without echo

  echo -ne "] rscript: Please repeat your luks password: \n"
...
;;
3) exit ;;
*) echo "] rscript: Wrong option please select again"; filesystem;;
esac
}
...
echo -ne "
] rscript: System detected your timezone to be '$time_zone' \n"
...
case ${options[$?]} in
    ...
    n|N|no|NO|No)
    echo "] rscript: Please enter your desired timezone e.g. Europe/London :"
    read new_timezone
...
keymap () {
echo -ne "
] rscript: Please select key board layout from this list"
...
echo -ne "] rscript: Your key boards layout: ${keymap} \n"
...
PS3='
] rscript: Select the disk to install on: '
...
userinfo () {
echo " "
echo "] rscript: Tip: You can navigate the installer with [wasd]"
read -p "] rscript: Please enter your username: " username
set_option USERNAME ${username,,} # convert to lower case as in issue #109 
while true; do
  echo -ne "] rscript: Please enter your password: \n"
  read -s password # read password without echo

  echo -ne "] rscript: Please repeat your password: \n"
  ...
  if [ "$password" = "$password2" ]; then
    ...
    echo -e "\n] rscript: Passwords do not match. Please try again. \n"
  fi
done
read -rep "] rscript: Please enter your hostname: " nameofmachine
...
aurhelper () {
  ...
  echo " "
  echo -ne "] rscript: Please enter your desired AUR helper:\n"
...
desktopenv () {
  ...
  echo " "
  echo -ne "] rscript: Please select your desired Desktop Enviroment:\n"
...
installtype () {
  echo " "
  echo -ne "] rscript: Please select type of installation:\n
